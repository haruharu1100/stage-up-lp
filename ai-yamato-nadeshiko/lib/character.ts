 ...
 *
 * ★★ここの数値はすべて「暫定値」（2026-08-24 人間が決定）★★
 *   0.80 や 0.85 という数字の意味は、どの測定方式を使うかで完全に変わる。
 *   顔embeddingの種類が違えば、同一人物でも 0.5〜0.7 に収まることもあれば 0.9 を超えることもある。
 *   測定方式を決めずに数字だけ先に固定すると、
 *   「厳しすぎて全部落ちる」か「緩すぎて別人が通る」かのどちらかになる。
 *
 *   → 実データ（同一人物 / 別人 / 似ている別人）で分布を測る Calibration を通してから確定する。
 *      手順と条件は docs/13_顔固定とCalibration仕様.md に固定してある。
 *      変更するときは Before / After / 理由 / dataset / FAR / FRR / F1 / HardNegative FAR を
 *      必ずObsidianへ残す。「なんとなく 0.80 → 0.75」は禁止。
 *
 *   ★Calibration 後は PASS / FAIL の2段階ではなく PASS / REVIEW / FAIL の3段階へ拡張する。
 *     ぎりぎりの一致を「合格」に丸めないため。
 */
