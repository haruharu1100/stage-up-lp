...
export const SCHEMA: string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS live_first_product_audit (
    ...
    --   「元レスポンス → 正規化後 → 画面表示値 を3段で並べて比較する」
    --   「価格だけでなく、送料・在庫・通貨・画像・URL・商品IDも同じ1件について追跡する」
    --   field_trace は全項目ぶんの3段（状態・変換ルール込み）をそのまま JSON で持つ。
    field_trace TEXT,
    -- 取れなかった項目名の一覧。1つでもあれば「正常取得」と表示してはいけない。
    incomplete_fields TEXT,
    -- 人が目で確認すべき不整合の件数。★1件でもあるうちは5件テストへ進まない。
    mismatch_count INTEGER,
    -- 追跡した項目が全部そろったか（1 = 正常取得と言ってよい / 0 = 言ってはいけない）
    fully_captured INTEGER,
    created_at TEXT NOT NULL
  )`,
...
export const COLUMN_ADDITIONS: Record<string, Record<string, string>> = {
  ...
  live_first_product_audit: {
     ...
     *
     *   ★ここも DEFAULT を付けない。
     *     mismatch_count に DEFAULT 0 を付けると、まだ検査していない古い記録が
     *     「不整合0件＝先へ進んでよい」に化ける。それは一番やってはいけない事故。
     */
    /** 全項目ぶんの3段（元値・状態・変換ルール・正規化後・表示値）をそのまま JSON で */
    field_trace: `TEXT`,
    /** 取れなかった項目名の一覧 */
    incomplete_fields: `TEXT`,
    /** 人が確認すべき不整合の件数。★1件でもあるうちは5件テストへ進まない */
    mismatch_count: `INTEGER`,
    /** 1 = 全項目そろった（正常取得と言ってよい） / 0 = 言ってはいけない */
    fully_captured: `INTEGER`,
  },
};
