...
| 区分 | 収益 | 費用 |
| --- | --- | --- |
| CONTENT | 有料noteの手取り | コンテンツ制作費・広告費・制作時間（時給2,500円換算） |
| SAAS | AIシステムの売上・MRR | 開発費・AI利用料・検索API利用料・サーバー費・ドメイン費・営業費・電話費・メール配信費・サポート費・ツール利用料・その他 |
| TOTAL | 上2つの合計 | 上2つの合計 |
...
`development` / `ai_api` / `search_api` / `server` / `domain` / `content` /
`advertising` / `sales` / `phone` / `email` / `support` / `tool` / `other`

AI利用料と検索API利用料は請求元が違うので分けてある。
「その他」に寄せると、後から何に使ったのか分からなくなり、減らしようが無くなる。

費目名を変えた時に過去の記入が無視されると、費用が実際より安く見える。
そのため `LEGACY_COST_KINDS`（`CONTENT_PRODUCTION` → `CONTENT` など）で旧名も読める。

検索API利用料は、台帳への手書きが無くても実際の検索回数から自動で SAAS 側に乗る。
見積り額（仮の単価×回数）と実額（請求書を見て人が入れる）は別々に持ち、実額が未入力なら 0 にしない。

### LABOR COST（自分が動いた時間）

台帳の `minutes` 列に書いた作業時間を、仮の時給 2,500円で費用に換算する。

```
LABOR_COST = 作業分数 ÷ 60 × 時給
```

現金が出ていかない時間を0円のままにすると「手作業なら無料」という結論になり、
赤字の事業が黒字に見える。時給は仮置きなので、金額だけでなく
「何時間をいくらで見積もったか」を必ず一緒に表示する。
...
## 15. 本番テスト開始前チェック（READY / NOT READY）

12項目すべてが揃って初めて `READY`。**1つでも欠けたら `NOT READY`。**

| # | 確認すること | 欠けていると何が起きるか |
| --- | --- | --- |
| 1 | 日本市場調査 | 海外で流行っただけの物を日本で売ることになる |
| 2 | 競合 | 高すぎて売れないか、安すぎて赤字になる |
| 3 | 価格 | いくら売れれば黒字かも決まらない |
| 4 | ターゲット | 反応が悪かった理由が分からない |
| 5 | チャネル | 必要な件数も費用も決まらない |
| 6 | テスト単位 | 「100件」が何の100件か分からず、結果を比べられない |
| 7 | サンプル数 | たまたまの結果を実力と勘違いする |
| 8 | 最大損失額（MAX_TEST_LOSS） | 負けを取り返そうとして損失が伸びる |
| 9 | 撤退条件（Kill Criteria） | 後から都合よく条件を緩めてしまう |
| 10 | 拡大条件（Scale Criteria） | 当たった時に活かせない |
| 11 | 損益計算（P&L） | 売れているのに赤字という状態に気付けない |
| 12 | 計測方法 | 良かったのか悪かったのか永久に分からない |

判定の細かい条件：

- 日本市場調査は「調べた」だけでは足りない。**情報源が2種類以上**必要
  （1つの検索エンジンだけで「日本に競合がいない」と断定しないため）
- 競合数は未調査（`null`）と0社を区別する。未調査は `NOT READY`
- 目標件数が、判断に必要な件数に届いていなければ `NOT READY`
- 撤退条件・拡大条件は10文字以上。「だめなら止める」のような、
  後から解釈を変えられる書き方は認めない
- P&L に未記入の費目が1つでもあれば `NOT READY`

**`READY` は「人が始めてよいか判断できる状態」という意味であり、
このシステムが投稿・送信・架電・課金を行うという意味ではない。**
その処理はこのリポジトリに存在しない。
...
このシステムは案件に点数を付けて順位を決めているが、
**その点数の付け方が正しいという証拠はまだ1件も無い。**
...
予測は実測より先に保存し、後から予測値を書き換えられないようにしてある。
実測が無い予測は「誤差0」ではなく、答え合わせが済んでいないものとして数から外す。

### 採点式を書き換えてよい条件

| 条件 | 値 | 理由 |
| --- | --- | --- |
| 答え合わせが済んだ予測の件数 | 20件以上 | 数件の当たり外れを法則だと思い込まないため |
| 率の指標1件あたりの母数 | 30件以上 | 30件未満の率は偶然で大きく動くため |
| 確度の区間1つあたりの件数 | 10件以上 | 区間ごとの達成率を出せるだけの数が要るため |

条件を満たすまでの判定は `INSUFFICIENT_DATA`。**この間は採点式を変更しない。**

判定は、言っている確度と実際の達成率の平均のズレで決める。

| 判定 | 意味 |
| --- | --- |
| `WELL_CALIBRATED` | ズレが±10pt以内。確度の付け方は妥当 |
| `OVERCONFIDENT` | 言った確度より実際の達成率が低い（甘く付けている） |
| `UNDERCONFIDENT` | 言った確度より実際の達成率が高い（辛く付けている） |

顧客獲得費（CAC）だけは「小さいほど良い」ので、達成の向きが逆になる。
