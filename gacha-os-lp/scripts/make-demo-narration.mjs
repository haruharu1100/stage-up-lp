 ...
 * text … 読み上げる文
 *
 * ★場面の切り替わり（gacha-os-demo.ja.vtt より／2026-08-27 の約48秒版）
 *    0.00 CUT1 ひとつの画面へ／ 2.46 一元管理の札／ 4.52 CUT2 設計値だけでは／
 *    6.96 CUT3 3つの還元率／10.82 毎日監視の札／12.91 CUT4 バックテスト／
 *   15.36 赤字の条件の札／16.71 CUT5 日々の運営／19.88 引く／23.46 マイページ／
 *   25.09 発送依頼／27.11 一次回答／30.26 人へ回る／32.06 下書きを送る／
 *   36.14 数字が動く／38.15 CUT6 監査／40.60 監査ログの札／41.96 CUT7 制作期間／
 *   45.11 LAST
 *
 ...
 *
 * 5) 「AI」を連発しないこと（2026-08-27 追加）。
 *    売っているのはオンラインガチャの制作・運営システムです。
 *    自動で答える所は「自動で」と言えば足ります。
 *    声のたびに「エーアイが」と付けると、
 *    中身のない売り文句のように聞こえます。
 */
const LINES = [
  { at: 0.45, max: 3.40, text: "運営に必要な画面を、ひとつにまとめました。" },
  { at: 4.75, max: 2.15, text: "予定だけでは、足りません。" },
  { at: 7.30, max: 3.20, text: "実際にいくら返したかまで、毎日見ています。" },
  { at: 13.25, max: 3.10, text: "先に試して、危ない条件を見つけます。" },
  { at: 17.10, max: 2.60, text: "お客様の画面と、並べて見ます。" },
  { at: 20.40, max: 3.00, text: "引いた分が、その場で左に入ります。" },
  { at: 23.80, max: 3.25, text: "当たった品も、発送の依頼も、同じ画面です。" },
  { at: 27.55, max: 2.55, text: "よくある質問には、自動で答えます。" },
  { at: 30.50, max: 2.60, text: "判断が要る件だけ、人に回ります。" },
  { at: 33.20, max: 2.80, text: "下書きは自動、送る判断は人です。" },
  { at: 36.40, max: 1.70, text: "集計を待ちません。" },
  { at: 38.55, max: 3.20, text: "操作の記録は、すべて残ります。" },
  { at: 42.40, max: 2.55, text: "期間は、内容によって変わります。" },
  { at: 45.60, max: 2.35, text: "くわしくは、ご相談ください。" },
];
