...
export type Flash = {
  ...
  /**
   * 何番目の知らせか。1から始まり、知らせが出るたびに1つ増えます。
   *
   * ═══════════════════════════════════════════════
   * ★なぜ番号が要るのか
   * ═══════════════════════════════════════════════
   *
   *   知らせは数秒で自分から消えます。
   *   消す係は「知らせが変わったら、数え直す」で動きます。
   *
   *   ところが、同じ文の知らせが続けて出ることがあります。
   *   ガチャを2回まわせば「C賞でした」が2回出ます。
   *   文だけで見ていると、2回目は「変わっていない」ことになり、
   *   1回目に始めた数え直しがそのまま進みます。
   *   すると2回目の知らせは、出た直後に消えます。
   *
   *   番号を付けておけば、文が同じでも別の知らせだと分かります。
   *
   * ★時刻（Date.now）を使わないこと。
   *   同じミリ秒に2つ出ると、また同じ値になります。
   *   それに、時刻を状態に入れると、
   *   同じ操作をしても毎回ちがう状態になり、テストが書けなくなります。
   */
  at: number;
};
...
export function reducer(s: ConsoleState, a: ConsoleAction): ConsoleState {
  ...
  return {
    ...
    flash: {
      ...
      /* ★番号を振るのも、ここ1か所だけにすること。
           知らせを作っている場所は30か所近くあります。
           そこへ1つずつ手で書き足せば、必ず書き忘れが出ます。
           書き忘れた知らせは、番号が無いので「変わっていない」と
           見なされ、消えずに次の画面まで居座ります。 */
      at: (s.flash?.at ?? 0) + 1,
    },
  };
}
...
/**
 * 知らせが自分から消えるまでの時間（ミリ秒）。
 *
 * ═══════════════════════════════════════════════
 * ★なぜ消すのか
 * ═══════════════════════════════════════════════
 *
 *   出したままにすると、「C賞でした」が次の画面にも、
 *   その次の画面にも付いてきます。
 *   お客様は、いま見ている画面の話だと思って読みます。
 *   ガチャの結果が、発送の画面の上に出ていたら、
 *   何かの手続きが終わったのだと勘違いします。
 *
 * ★短くしすぎないこと。
 *   日本語で40字ほどの文を、目で追って理解するまでに数秒かかります。
 ...
 *   消えるものは、見逃されます。
 *   後から確かめる必要がある事実は、必ず
 *   お知らせ一覧か監査ログにも残します。この帯は「今の合図」です。
 */
