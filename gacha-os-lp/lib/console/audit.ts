...
export type AuditAction =
   ...
   * ═══════════════════════════════════════════════════════
   *
   * ★これを「ただの設定変更」として SETTINGS_CHANGE に混ぜないこと。
   *
   *   ここで変えるのは、特定商取引法に基づく表記・利用規約・
   *   プライバシーポリシー・運営法人の名前と住所です。
   *
   *   これらは「画面の設定」ではありません。
   *   ★お店が、お客様に対して行っている法的な表示そのものです。
   *
   *   後から必ず問われます。問われるのは、たいてい返品や苦情のときです。
   *
   *       「返品できないと書いてあった覚えがない」
   *         → いつ、誰が、返品の記載をどう変えたか
   *       「この住所に送ったのに、戻ってきた」
   ...
   *         → 変えた前と後
   *
   * ★長い本文を、まるごと before / after に入れないこと。
   *   監査ログが読めなくなります。読めない記録は、誰にも使われません。
   *   残すのは「どの項目を触ったか」と「文字数がどう変わったか」までです。
   */
   ...
   * ガチャの棚（カテゴリ）の出し入れ。
   *
   * ★DELETE を CREATE と分けて残すこと。
   *   棚を消すと、その棚に入っていたガチャが、
   *   お客様の絞り込みからまとめて消えます。
   *   「昨日まで売れていたのに、今日から売れない」を調べるとき、
   *   最初に見るのが、この行です。
   *
   * ★ASSIGN（どのガチャを、どの棚に置いたか）も残すこと。
   *   棚そのものは無事でも、置き場所が変わればお客様には届きません。
   */
  | "GACHA_CATEGORY_CREATE"
  | "GACHA_CATEGORY_RENAME"
  | "GACHA_CATEGORY_DELETE"
  | "GACHA_CATEGORY_ASSIGN"

  /**
   * 賞の「呼び名」を変えた記録（020）。
   *
   * ★これを SETTINGS_CHANGE に混ぜないこと。
   *
   *   ここで変わるのは、お客様の当選画面に出る文字そのものです。
   *   「S賞」を「特賞」に改めれば、その瞬間から、
   *   売り場も、当選画面も、過去の当選一覧も、新しい呼び名になります。
   *
   *   後から必ず問われます。
   *
   *       「当たったときは特賞と出ていたのに、いまは1等になっている」
   *         → いつ、誰が、どの等級の呼び名を、何から何へ変えたか
   *
   * ★中の記号（S / A / B / C / D）は変わりません。
   *   変わるのは見せる文字だけで、抽選・残り口数・当選履歴は動きません。
   *   ですので、この記録には
   *   「記号」と「変える前の呼び名」と「変えた後の呼び名」の3つを残します。
   *   呼び名だけ残すと、あとから、どの等級の話か分からなくなります。
   */
  | "GRADE_LABEL_UPDATE";
