 ...
 * 公開ボタンが永久に押せない状態でした。
 *
 * ① 判定に使う数字が「残数還元率」ではなく「その途中の最大値」だった
 *
 *    しきい値（105% / 110%）は「残数還元率がこれを超えたら危ない」という意味です。
 ...
 *        その停止ラインを見張るのが、相場ウォッチと実績還元率モニタです。
 */
...
export type Distribution = {
   ...
   * 「次に買うお客様から見た残数還元率」を測り、200通りの当選順で並べたもの。
  ...
  /**
   * 参考値：販売中に残数還元率がいちばん高くなった時点の値（％）の中央値。
...
const SCENARIOS: {
  ...
  },
  {
     ...
     *   終盤に残ればその先30%の残数還元率は必ず110%前後になります。
...
type Run = {
   ...
   */
  normalRtp: number;
  /** 参考：通常運営の間に残数還元率がいちばん高くなった値（％）。判定には使わない */
  normalPeak: number;
  normalPeakAt: number;
  /**
   * 終盤（消化70〜100%）で到達した残数還元率の最大値（％）。
   * ただし「残り数口」の極端な値は含めない。
...
function simulate(
  ...
  // 画面には「残数還元率 0.0%＝とても安全」と出てしまう。これは嘘になる。
  let rtpSamples = 0;

  let normalPeak = 0;
  let normalPeakAt = 0;
  /** 通常運営の中で最後に測れた残数還元率＝消化70%（または販売停止）の時点の値 */
  let normalRtp = 0;
  let endgamePeak = 0;
  ...
  for (let k = 0; k < stopAt; k++) {
    ...
    // 料金が0円だと 0 で割ることになり、残数還元率が Infinity になる。
...
function combine(
  ...
  /* 悪い当選順（10回に1回）の残数還元率が極端なら、中央値がどれだけ良くても危険。
  ...
  if (d.rtpP90 >= THRESHOLD.tailExtreme) {
    return {
      verdict,
      reason: `当選順が悪い方から10%に入ると、消化70%の時点で残数還元率が ${d.rtpP90.toFixed(1)}% になります。上位賞に予算を寄せすぎているため、上位賞が残ったまま販売が進むと止めざるを得なくなります。上位賞の価値を下げるか、本数を増やして分散してください。`,
    };
  }
  ...
  if (verdict === "SAFE") {
    return {
      verdict,
      reason: `残数還元率は中央値 ${d.rtpMedian.toFixed(1)}%、悪い方から5%でも ${d.rtpP95.toFixed(1)}% にとどまります。赤字になった回はありませんでした。`,
    };
  }

  return {
    verdict,
    reason: `残数還元率が中央値で ${d.rtpMedian.toFixed(1)}%、悪い方から5%では ${d.rtpP95.toFixed(1)}% まで上がります。この付近で停止するか、景品を追加する判断が必要です。`,
  };
}
...
function runScenario(
  ...
  // 売れ残りが大きい場合は、残数還元率が良くても注意（資金が寝る）
  if (mid.leftoverValue > mid.revenue * THRESHOLD.leftoverRatio) {
    verdict = worseOf(verdict, "CAUTION");
    reason = `残数還元率は中央値 ${distribution.rtpMedian.toFixed(1)}% で問題ありませんが、売れ残った景品の価値が売上の半分を超えます。仕入れた資金がそのまま寝ることになります。`;
  }
...
function endgameReason(a: {
  ...
  const mid =
    a.topTailRate > 0 && a.rtpAtTail > 0
      ? `残ったまま残り${a.tailSlots}口になった場合、その時点の残数還元率は ${a.rtpAtTail.toFixed(0)}% になります。`
