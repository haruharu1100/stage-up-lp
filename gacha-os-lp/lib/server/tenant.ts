...
export const TENANT_TABLES = [
  ...
  /* お店の住所（ドメイン）と、賞の呼び名（020）。
     ★ここを一覧から外さないこと。
       tenant_domains      … どの住所（shop-a.example.com）が、どのお店かの対応表。
                             ★この表が、お客様の入口を決めています。
                               会社の壁が1枚抜けると、
                               「よその店の住所を、自分の店に付け替える」ことが
                               できてしまいます。そうなると、
                               よその店のお客様が、そのまま自分の店に流れます。
                               お金の表と同じ厳しさで扱います。
       tenant_grade_labels … 賞の呼び名（特賞 / 1等 / PSA10賞 など）。
                             ★中の記号（S/A/B/C/D）は動きませんが、
                               ここに入っている文字は、そのまま
                               お客様の当選画面に出ます。
                               会社の壁が1枚抜けると、
                               「よその店の当選画面の文字を書き換える」ことが
                               できてしまいます。 */
  "tenant_domains",
  "tenant_grade_labels",
