...
const signals = [
  {
    ...
    body: `最新の市場価格で計算した残数還元率が ${RTP_CAUTION}% を下回っています。この画面を見て、その日は何もしない、という判断ができます。`,
  },
  {
    code: "CAUTION",
    label: "確認推奨",
    ...
    body: `残数還元率が ${RTP_CAUTION}% 以上 ${RTP_DANGER}% 未満。景品構成・口数・販売速度のどれが効いているかを確認する候補として上がります。`,
  },
  {
    code: "DANGER",
    label: "要対応",
    ...
    lead: "その日のうちに決める場面です。",
    body: `残数還元率が ${RTP_DANGER}% 以上。販売停止・景品差し替え・口数調整のどれを取るかを選ぶ画面へ進みます。停止するかどうかを決めるのは運営者です。`,
  },
  {
    code: "UNKNOWN",
    label: "情報不足",
...
const unknownCauses = [
  ...
  {
    t: "景品の情報が足りない",
    d: "景品が未登録、価格の対象が特定できないなど、残数還元率の計算に届かない状態です。",
  },
];

/** 市場価格の3つの状態。折りたたみの中で原因と一緒に見せる */
const priceStates = [
  { c: "FRESH", d: "価格は新しい。残数還元率を計算して表示します。" },
  {
    c: "STALE",
          ...
          <div className="grid gap-6 p-5 sm:gap-8 sm:p-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:items-center lg:gap-12">
            ...
            <div className="rounded-2xl border border-edge bg-paper2 px-5 py-6 text-center sm:px-6 sm:py-7">
              ...
              <p className="mt-3 text-note text-slate2 sm:mt-4">
                {jp(
                  "残数還元率の欄は「—」になります。市場価格が FRESH でない限り、計算結果が",
                )}
