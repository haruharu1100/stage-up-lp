    ...
    <Act
      ...
      lead={
        <>
          ...
          AI GACHA OS では、複数の価格変動・当選シナリオを事前シミュレーション。
          公開前にリスクを確認できます。
        </>
      }
    >
      ...
      <div className="grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1.25fr)] lg:gap-10">
        {/* 左：設計上はこう見えている */}
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] px-8 py-9">
        ...
        {/* 右：実際に起きること */}
        <div
          className={`rounded-3xl border bg-danger/[0.06] px-8 py-9 ${surgeTone.ring}`}
        >
      ...
      <div className="mt-16 text-center">
        ...
        <p className="mx-auto mt-7 max-w-[36em] text-body text-pretty text-white/58">
          相場が 25% 上がるだけで、同じ構成が{" "}
          {signed(surge.distribution.profitMedian)} になります。
          公開してから気づいても、景品はもう出ていきます。
          だから、公開する前に試します。
        </p>
      </div>
      ...
      <div className="mt-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {rows.map(({ key, label, r }) => {
          ...
          return (
            <div
              key={key}
              className={`rounded-3xl border bg-white/[0.03] p-7 ${t.ring}`}
            >
              ...
              <p className="num mt-6 text-label text-white/35">実還元率 中央値</p>
      ...
      <div className="mt-16 grid gap-4 sm:grid-cols-3">
        {[
          ...
          <div
            key={x.t}
            className="rounded-3xl border border-white/8 bg-white/[0.02] px-7 py-7"
          >
            <p className="text-note font-bold text-white/80">{x.t}</p>
            <p className="mt-3 text-note leading-[1.95] text-white/50">{x.d}</p>
          </div>
        ))}
      </div>

      <div className="mt-14 flex flex-col gap-3.5 sm:flex-row sm:justify-center">
