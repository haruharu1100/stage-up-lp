...
/* ────────────────────────────────────────────────
   1. 価格急騰シミュレーション
   1回 3,000円 × 500口 のガチャ。
   S賞スニーカーの相場だけが動くと、残数還元率と粗利がどうなるか。
   ──────────────────────────────────────────────── */
...
/* ────────────────────────────────────────────────
   2. 残数シミュレーション
   1000口 → 800 → 500 → 200。
   同じガチャでも「何が残っているか」で残数還元率はまったく違う。
   ──────────────────────────────────────────────── */
  ...
  return (
    <div className="lp-card p-7 sm:p-9">
      ...
      <p className="mt-6 text-note text-slate2">
        ...
        {/* ★「残数ベース残数還元率」と重ねないこと。残数還元率が正式な名前です */}
        <span className="font-bold text-slate">{jp("残数還元率")}</span>
        が出ます。
      </p>
      ...
      */}
      ...
      >
        <p className="num text-label text-white/40">この時点の残数還元率</p>
...
export default function PriceShock() {
  return (
    <Section
      ...
      lead="採算が崩れるのは公開したあとです。相場が上がったときと、上位賞が残ったまま口数だけ減ったとき。その2つを動かして確かめられます。市場価格は、正規に利用できるデータソースと接続して定期的に取り込み、購入時価格との差がそのまま残数還元率の計算に入ります。"
    >
