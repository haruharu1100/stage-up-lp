...
const MAP: Record<Trouble, { name: string; why: string; modules: string[] }> = {
  build: {
    name: "AIガチャ設計＋残数還元率管理",
    why: "ガチャ1本あたりの設計時間が長いほど、公開できる本数が体制の上限で決まってしまいます。設計を「条件を決めて承認するだけ」にすると、本数を増やしても作業時間が比例しにくくなります。",
    modules: ["AIガチャ自動設計", "承認フロー", "設定・残数還元率"],
  },
  rtp: {
    name: "残数還元率管理＋市場価格連動",
    why: "還元率は公開したあとに動きます。上位賞が残ったまま口数が減ったとき、景品の相場が上がったとき。この2つを見ていないと、気づいたときには赤字のまま売り切れています。",
    modules: ["残数還元率", "市場価格ベース還元率", "しきい値超過の警告と販売停止"],
  },
  price: {
    name: "市場価格連動＋残数還元率管理",
    why: "登録した仕入値のままだと、実際の採算とズレていきます。相場を定期取得して残数還元率へ反映すると、採算が反転する前に手を打てます。",
    modules: ["商品価格の定期取得", "価格変動の通知", "全ガチャの残数還元率へ自動反映"],
  },
  ship: {
    name: "発送自動化＋AI顧客対応",
...
export default function Diagnose() {
  ...
  const result = useMemo(() => {
    ...
    if (gachaVol === "16〜30本" || gachaVol === "31本以上") {
      extra.push(
        "公開本数が多いため、1本ずつ手で還元率を確認する運用は負担が大きくなります。残数還元率の自動監視を優先することをおすすめします。"
      );
    }
    if (running === "no") {
      extra.push(
