...
test("⑩ 停止中の会員は、正しいパスワードでも通らない", async () => {
  ...
  /* ═══════════════════════════════════════════════════════
     ★2026-09-05、ここで求める文言を入れ替えました
     ═══════════════════════════════════════════════════════

       それまでは「パスワードが違います。」を求めていました。
       停止中であることを外へ教えないため、という理由です。

       その理由は、ログインの入口では、いまも正しいままです。
       ログインの入口は誰でも叩けます。
       「このアドレスは生きている」と分かれば、名簿が作れます。

       ですが、ここは違います。
       ここまで来られるのは、正しい入場券（クッキー）を持つ
       ご本人だけです。ご本人に、ご自分のアカウントの状態を
       伏せる理由はありません。

       伏せるほうが害があります。
       「パスワードが違います」と出すと、その方はパスワードを
       何度も入れ直し、再設定まで試し、それでも入れず、
       最後に問い合わせをされます。
       運営は、自分が止めた相手からの問い合わせを受けることになります。

       ★2026-09-05 に、門番（lib/server/context.ts）が
         お客様の行も毎回読むようになりました。
         停止は、すべての入口で同じ文言・同じ code で断ります。
         ここだけ別の文言にすると、
         「この入口だけ様子が違う」こと自体が手がかりになります。 */
  assert.equal(
    k.body.message,
    "このアカウントは、現在ご利用いただけません。",
    "停止中の会員に、状態と合わない案内を返しています",
  );
  assert.equal(
    k.body.code,
    "ACCOUNT_SUSPENDED",
    "停止で断ったことが、記録から読み取れません",
  );
});
