...
test("1口のガチャでも、残数還元率が 0.0%（＝とても安全）とは表示されない", () => {
  ...
  assert.ok(
    ...
    `設計還元率は ${designedRtp(oneSlot).toFixed(1)}% なのに、残数還元率が ${normal.distribution.rtpMedian}% と出ています`
  );
  // 設計還元率（99.5%）とほぼ同じ値になるはず
  assert.ok(
    Math.abs(normal.distribution.rtpMedian - 99.5) < 0.5,
    `残数還元率 ${normal.distribution.rtpMedian}% が設計還元率 99.5% から離れすぎています`
  );
  // 設計上の還元率も正しく 99.5%
