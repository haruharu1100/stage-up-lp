 ...
 *   ③ 残数還元率の式が、還元率モニタ（RtpMonitor）と同じであること
...
test("残数還元率の式は「残っている景品の合計価値 ÷（残り口数 × 料金）」", () => {
  const g = g0();
...
test("残っている景品の本数の合計 ＝ 残り口数（1口引けば景品が1つ出るので）", () => {
  for (const g of CATALOG) {
    ...
    assert.equal(
      stock,
      g.left,
      `${g.title}：残り口数 ${g.left} に対して景品が ${stock}。これがずれると残数還元率が嘘になります`,
    );
...
test("デモに出す残数還元率は 90〜105% に収まる（相場から外れた数字を出さない）", () => {
  for (const g of CATALOG) {
    const v = realRtp(g);
    assert.ok(
      v >= 90 && v <= 105,
      `${g.title}：残数還元率 ${v.toFixed(1)}% は、お客様に見せる数字として不自然です`,
    );
  }
});
