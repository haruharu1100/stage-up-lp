...
export const KEEPA_FIELD_SPECS: KeepaFieldSpec[] = [
  ...
  {
    path: 'monthlySold',
    labelJa: 'Keepaの月間購入回数（「◯個以上」の区分値）',
    expected: ['number', 'null', 'missing'],
    sentinels: DEFAULT_SENTINELS,
    unit: 'COUNT',
    noteJa:
      '★呼び方に注意。実際に売れた数を測ったものではないが、「推定」でもない。'
      + '公式説明は、Amazonの検索結果に出る「先月◯点購入されました」をそのまま渡したもので、'
      + 'Amazon側が "10+" "100+" のような粗い段階でしか出さないため、'
      + '意味は「少なくともその数」＝**下限**である。ちょうどの数ではないので割り算の材料にしない。'
