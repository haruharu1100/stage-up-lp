...
export const VENUE_RESEARCH: VenueResearch[] = [
  {
    ...
    note:
      ...
      + '＝相場データは18ヶ月で消す仕組みが要る。'
      /*
       * ★2026-08-22・ご本人の判断で見送りになった。
       *   調査の点数（77点・Gate通過）はそのまま残す。事実は変わっていないから。
       *   変わったのは「いま進むかどうか」だけ。ここを混ぜて点数を下げると、
       *   あとで再検討するときに「なぜ低いのか」が分からなくなる。
       */
      + '／【2026-08-22 見送り】ご本人の判断により、当面 SP-API へは進まない。'
      + '理由は「まず商品が売れるかどうかを Keepa で確かめる」ため。'
      + '調査結果（77点・Gate通過）はそのまま残す。事実は変わっておらず、変わったのは進む順番だけ。'
      + '再検討するときは、上の未確認3件から始めること。',
  },
  {
   ...
   *
   * 【なぜ市場でもないのにここに載せるのか】
   * ご本人の判断で「売れるかどうかのテストに Keepa を使う」と決まった。
   * 使うと決めたものを一覧に載せないと、規約の確認が永久に始まらない。
   * ルール57・65・67 は市場だけの話ではなく、**外から数字を持ってくる相手すべて**に当てはまる。
   *
   * 【Keepa は Amazon ではない】
...
/*
 * ★2026-08-22 更新（その2）。ご本人の判断が出たので書き換えた。
 *
 *   ご本人：「一旦これが売れるものなのかどうかテストするので Keepa を使用して」
 *   → Amazon SP-API は見送り。Keepa を使い続ける。
 *
 * 【この判断で規約上どうなるか】
 * AUP 4.3（外部データサービスの使用禁止）は **SP-API を使い始めてはじめて掛かる**条項なので、
 * SP-API に手を出さない以上、Keepa を使い続けること自体に規約上の問題は生じない。
 *
 * 【ただし「決まった」のは見送りだけ】
 * 第1自動市場そのものは、まだ決まっていない。
 * SP-API を外した結果、Gate を通った候補は0件になった。
 * ここで「候補が無いから2位を繰り上げる」ということはしない（Gateの意味が無くなる）。
 */
export const FIRST_LIVE_CONNECTOR_STATUS_JA =
  '第1自動市場は未確定です。'
  + '2026-08-22、ご本人の判断で Amazon SP-API は見送りになりました（まず「売れるかどうか」を Keepa で確かめるため）。'
  + '大口出品は契約済みで追加費用は発生しませんが、規約本文を読んだ結果'
  + '①AUP 4.3 により既存サービス（Keepa）を併用できなくなる ②カタログAPIが商品ページURLを返さないため'
  + '「購入ページを開く」導線を作れない、の2点が確定したためです。'
  + 'SP-API を外したので、いまGateを通っている候補は0件です。'
  + '繰り上げはしません。次の候補は Yahoo!ショッピング商品検索v3で、商用利用の可否を確認したうえで、'
  + '第1自動市場はご本人と一緒に決めます。'
  + 'なお Keepa は自動取得の相手ではありません。ご本人が画面を見て数字を書き写す使い方（HUMAN_ENTRY）で、'
  + 'システムは Keepa へ一切アクセスしません。';
