...
export function deriveFeeStatus(f: FeeStatusInput, now = Date.now()): FeeStatusResult {
  ...
  if (!hasSource && !hasVerifiedAt) {
    return { status: 'UNKNOWN', note: '実額とされているが、出典も確認日も無い。裏付けが取れない', verifiedAgeDays };
  }
  if (!hasSource) {
    return { status: 'UNKNOWN', note: '確認日はあるが、出典（料金ページ・契約書など）が無い', verifiedAgeDays };
  }
  if (!hasVerifiedAt) {
    return { status: 'UNKNOWN', note: '出典はあるが、いつ確認したのかが分からない', verifiedAgeDays };
  }
  ...
  return {
    status: 'VERIFIED',
    note: `出典と確認日が揃っている（${Math.round(verifiedAgeDays ?? 0)}日前に確認）`,
    verifiedAgeDays,
  };
}
...
export function capFeeConfidence(raw: number, status: FeeStatus): number {
  return Math.min(raw, FEE_CONFIDENCE_CAP[status]);
}
...
export async function syncFeeStatuses(now = Date.now()): Promise<Record<FeeStatus, number>> {
  const rows = await all(
    `SELECT id, is_estimated, source_url, source_name, source_type, verified_at, effective_from, effective_to
       FROM venue_fee_profiles`,
  );
