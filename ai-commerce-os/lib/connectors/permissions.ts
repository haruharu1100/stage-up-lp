...
export function mayStartAutoCollection(r: VenueResearch): { ok: false; reasonJa: string } | { ok: true } {
  ...
  /*
   * 【2026-08-25 追加：8項目だけを見ていたのは穴だった】
   *
   * ここは元々「8つの許可項目に UNKNOWN が無いか」しか見ていなかった。
   * だが Keepa のように、8項目は規約本文で全部埋まったのに、
   * 「正規化して保存してよいか（U1）」「契約終了後に持っていてよいか（U2）」のような
   * **8項目に収まらない問い**が残っている相手がいる。
   * それを `unknowns` に書いておきながら判定で無視していたら、書いた意味が無い。
   *
   * この関数の見出しは「1つでも分からないことが残っていたら、自動取得を始めない」である。
   * その通りに、残っている問いの数もそのまま見る。
   */
  if (r.unknowns.length > 0) {
    return {
      ok: false,
      reasonJa: `許可項目は埋まっているが、まだ答えの出ていない問いが ${r.unknowns.length} 件あるため、接続しません。`,
    };
  }
  return { ok: true };
}
