...
import {
  canon, checkpointFor, listingKeyOf, parseListingState, workStepJa,
  LISTING_STATE_JA, TRACK_CHECKPOINTS,
...
const ALIASES: Record<string, string[]> = {
  ...
  entry_seconds: ['entry_seconds', 'seconds', '入力秒数', '入力時間', '作業秒数'],
};
...
export async function importListingObservations(text: string): Promise<ObservationImportResult> {
  const out: ObservationImportResult = {
    ...
    caution: null, entrySecondsTotal: 0, entrySecondsRows: 0,
  };
  ...
  for (let i = 1; i < grid.length; i++) {
    ...
    const seconds = get('entry_seconds') === '' ? null : Number(get('entry_seconds'));
    ...
    if (isFirst) {
      ...
      stmts.push({
        ...
        args: [
          ...
          cleanText(get('sold_at')) || null, now, key,
        ],
      });
...
export async function recordWork(
  batchId: string, step: WorkStepCode, seconds: number, rows: number, note?: string,
): Promise<void> {
  if (!Number.isFinite(seconds) || seconds <= 0) return;
  await run(
    `INSERT INTO observation_work_log (batch_id, step, seconds, rows, note, recorded_at)
     VALUES (?,?,?,?,?,?)`,
    [batchId, step, seconds, Math.max(0, Math.trunc(rows)), note ?? null, nowIso()],
  );
}
