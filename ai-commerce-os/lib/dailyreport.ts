...
export function decideNextAction(
  ...
  if (autoFetchVenues === 0 && realShadowCount === 0) {
    // 【2026-08-20 訂正】以前ここに「人の手による記録は規約上いちばん安全な方法です」と書いていたが、これは誤り。
    // CSVという形式は根拠にならない。自分で画面を見て記録したものは「提供を受けたデータ」ではない（SELF_OBSERVED）。
    // 当座の手段としては使えるが、正規の提供手段を探す作業と同時に進めるものとして案内する。
    return '正規の取得手段が1件も無いのが最大の詰まりです。まずは1つの市場を選び、'
      + '実際の公開画面を見て相場を手で記録し、取得方法 MANUAL_OBSERVATION として取り込んでください。'
      + 'ただしこれは「事業者から提供を受けたデータ」ではありません（自動巡回はしない・閲覧の範囲を超えない）。'
      + '同時に、その市場の公式API・正式なデータフィードの有無も確認してください。';
  }
