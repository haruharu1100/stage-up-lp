...
export const EXCLUSION_RULES: ExclusionRule[] = [
  ...
  },
  {
    ...
    severity: 'HARD_BLOCK',
    patterns: [
      // ★「週5日以上」「週5日勤務」だけを見ていると、「週5日・1日8時間」「週5日〜」のような
      //   ごく普通の書き方を全部見逃す。実際の募集文で日数のうしろに続く言葉は決まっていない。
      //   「週4〜5日」を「週5日」と書くことはないので、日数だけで拾って構わない。
      /週\s*[45５４]\s*日/,
      /週\s*[45５４]\s*(以上|固定|フル)/,
      /シフト\s*制/,
      /月〜金/,
  ...
  },
  {
    ...
    patterns: [
      ...
      // ★ここも「必須」と書かれるとは限らない。道具の名前と「入れる／使う」がそろえば拾う。
      /(時間管理|勤怠|稼働管理|作業時間|稼働時間)\s*(の)?\s*(管理)?\s*(ツール|ソフト|アプリ|システム)\s*(を|の)?\s*(導入|インストール|入れて|入れる|利用|使用)/,
  ...
  },
  {
    ...
    patterns: [
      ...
      // ★「導入が必須」まで書いてある募集はむしろ少ない。「監視ツールを入れていただきます」
      //   のように、さらっと1行で書かれるほうが普通なので、道具の名前が出た時点で拾う。
      /(監視|モニタリング|勤怠管理)\s*(ツール|ソフト|アプリ|システム)\s*(を|の)?\s*(導入|インストール|入れて|入れる|利用|使用|常駐)/,
      /(PC|パソコン|端末)\s*監視/,
