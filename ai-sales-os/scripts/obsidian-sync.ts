...
1. **gBizINFO** と **法人番号Web-API** の鍵を入れる（どちらも無料）。ここが入るまで本物の会社データが1件も取れません。
2. \`npm run companies:fetch\` で実際の会社を集め、\`npm run companies:verify\` でHPの照合結果を見る。
3. 承認画面で、実際に送る文面を人が読む。**この時点ではまだ送りません。**
4. 1件目に選ぶ操作を**1つだけ**決める。問い合わせフォームが最も無難です（相手の同意が要らず、失敗しても電話ほど印象を悪くしません）。
